...
function generatePdf(payload, outPath) {
  return new Promise((resolve, reject) => {
    const pyScript = path.join(process.cwd(), "backend", "generators", "pdf_generator.py");
    ...
    proc.on("close", (code) => {
      ...
      // Some generator versions emit raw PDF bytes to stdout.
      if (stdoutBuffer.slice(0, 4).toString() === "%PDF") {
        fs.writeFileSync(outPath, stdoutBuffer);
        return resolve({ success: true, mode: "stdout-pdf" });
      }
